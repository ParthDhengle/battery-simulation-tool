...
if __name__ == '__main__':
    ...
    I_module = setup_data['I_module']  # From setup_data (add this line before run_electrical_solver)
    ...
    axs[2].set_ylim(-1, max(qgen_cell0) * 1.1 if max(qgen_cell0) > 0 else 1)  # Auto-scale with buffer
    axs[2].legend(loc='upper right')

    # Module Current plot (added for clarity on activity)
    axs[3].plot(time_days[:len(I_module)], I_module, color='purple', label='Module Current')
    ...
    vacation_periods = [  # Approximate day ranges based on rules
        (90, 92), (101, 102),  # Apr 1-2, 13-14 (day 91=Apr1, etc.; adjust for 0-index)
        (181, 186), (209, 210),  # Jul 3-5,29-30
        # Add for Aug/Dec if needed, but rule mismatch means default cycles
    ]
